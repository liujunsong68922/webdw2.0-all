...
function WebdwUI_callback(obj) {
	...
	if(status == 200){
		_webdwui.uuid = jsonobj.uuid;
		_webdwui.uiArray = jsonobj.uiobjList;
		_webdwui.removeAllChild();
		_webdwui.drawAllChild();	
		alert(jsonobj.message);
	}else{
		console.log(jsonobj.message);
		alert("Message:" + jsonobj.message);
	}
}
...
const CWebDWUIView = {
	...
	setDataobject(token,dwname){
		var surl="http://localhost/webdw/setdataobject?token="+token+"&dwname="+dwname;
		// 增加随机数
	...
	retrieve(token,dwname,args){
		var surl="http://localhost//webdw/retrieve?token="+token+"&dwname="+dwname+"&args="+args;
		// 增加随机数
		surl = surl+"&rand="+(Math.random()*100);
